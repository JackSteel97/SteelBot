...
public static class Interactions
{
    public static class Pets
    {
        ...
        public static TextInputComponent NameInput(long petId, string placeholderSuffix) => new TextInputComponent("Name", $"{InteractionIds.Pets.NameInput}:{petId}", $"Name this {placeholderSuffix}...", min_length: 1, max_length: 70);

        public static TextInputComponent MovePositionInput(long petId, int maxInput) => new TextInputComponent("Move To Position", $"{InteractionIds.Pets.MovePositionInput}:{petId}", $"Enter a position between 1 and {maxInput}...", min_length: 1, max_length: 2);

        public static DiscordButtonComponent Manage(long petId, string name) => new DiscordButtonComponent(ButtonStyle.Primary, $"{InteractionIds.Pets.Manage}:{petId}", $"Manage {name}");

        public static DiscordButtonComponent Replace(long petId, string name) => new DiscordButtonComponent(ButtonStyle.Danger, $"{InteractionIds.Pets.Replace}:{petId}", $"Replace {name}");
        public static DiscordButtonComponent Treat(long petId, string name) => new DiscordButtonComponent(ButtonStyle.Success, $"{InteractionIds.Pets.Treat}:{petId}", $"Treat {name}");
    }
