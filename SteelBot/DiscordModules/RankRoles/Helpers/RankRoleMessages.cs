...
public static class RankRoleMessages
{
    ...
    public static DiscordMessageBuilder RoleAlreadyExistsForLevel(int requiredRank, string existingRoleName) => new DiscordMessageBuilder().WithEmbed(EmbedGenerator.Error($"A rank role already exists for level {Formatter.InlineCode(requiredRank.ToString())} - {Formatter.Bold(existingRoleName)}, please delete the existing role."));

    public static DiscordMessageBuilder RoleDoesNotExistOnServer(string roleName) => new DiscordMessageBuilder().WithEmbed(EmbedGenerator.Error($"The Role {Formatter.Bold(roleName)} does not exist. You must create the role in the server first."));

    public static DiscordMessageBuilder RankRoleCreatedSuccess(string roleName, int requiredRank, string alreadyAchievedUsers) => new DiscordMessageBuilder().WithEmbed(EmbedGenerator.Success($"{Formatter.Bold(roleName)} set as a Rank Role for Rank {Formatter.Bold(requiredRank.ToString())}\n\n{alreadyAchievedUsers}", "Rank Role Created!"));
