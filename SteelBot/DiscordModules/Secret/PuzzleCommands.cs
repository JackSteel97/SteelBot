...
public class PuzzleCommands : TypingCommandModule
{
    ...
    {
        _numberWords = new List<string>();
        for (int i = 1; i <= _numberOfQuestions; i++)
        {
            _numberWords.Add(i.ToWords());
        }
    ...
    {
        int puzzleIndex = _numberWords.FindIndex(word => word.Equals(context.Channel.Name, StringComparison.OrdinalIgnoreCase));
        if (puzzleIndex >= 0)
        {
    ...
    public async Task Answer(CommandContext context, [RemainingText] string answer)
    {
        ...
        if (puzzleIndex >= 0)
        {
            ...
            else
            {
                ...
                var commonServer = await context.Client.GetGuildAsync(civlationId);
    ...
    private static async Task AdvanceUserToNextLevel(CommandContext context, int puzzleNumber)
    {
        ...
        var nextRole = context.Guild.Roles.Values.FirstOrDefault(role => role.Name.Equals($"puzzle-{nextPuzzleNumber.ToWords()}", StringComparison.OrdinalIgnoreCase));
    ...
    private async Task PostPuzzle(CommandContext context, int puzzleNumber)
    {
        ...
        switch (puzzleNumber)
        {
            ...
            case 12:
                var imageStream = File.OpenRead(Path.Combine(_appConfigurationService.BasePath, "Resources", "Puzzle", "Cow.jpg"));
                var zipStream = File.OpenRead(Path.Combine(_appConfigurationService.BasePath, "Resources", "Puzzle", "Inside.zip"));
            ...
            case 23:
                var imageStream23 = File.OpenRead(Path.Combine(_appConfigurationService.BasePath, "Resources", "Puzzle", "LookUp.jpg"));
                var excelStream23 = File.OpenRead(Path.Combine(_appConfigurationService.BasePath, "Resources", "Puzzle", "OhNo.xlsx"));
    ...
    {
        var fs = new FileStream(Path.Combine(_appConfigurationService.BasePath, "Resources", "Puzzle", fileName), FileMode.Open, FileAccess.Read);
